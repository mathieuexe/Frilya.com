-- Assurons-nous que la table existe et a la bonne structure
...
DROP POLICY IF EXISTS "Users can insert their own tickets or anonymous tickets" ON report_tickets;
DROP POLICY IF EXISTS "Admins can view and update all tickets" ON report_tickets;
DROP POLICY IF EXISTS "Users can view their own tickets" ON report_tickets;
DROP POLICY IF EXISTS "Anyone can insert tickets" ON report_tickets;

-- Politique pour PERMETTRE À TOUT LE MONDE (connecté ou anonyme) de créer un ticket
CREATE POLICY "Anyone can insert tickets"
ON report_tickets FOR INSERT
WITH CHECK (true);

-- Politique pour permettre aux utilisateurs connectés de VOIR LEURS PROPRES tickets
